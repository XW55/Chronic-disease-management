...
const request = (option, header = {}) => {
  ...
  !option.hideLoading && uni.showLoading({ mask: true })
  header = {
    'Content-Type': 'application/json',
    dataType: 'json',
    ...header,
  }
  if (option.auth !== false) header.Authorization = uni.getStorageSync('token') || ''
    ...
    uni.request({
      ...
      success: (res) => {
        ...
        if (data.code === 401) {
          ...
          uni.reLaunch({ url: '/loginSign/pages/login/login' })
          setTimeout(() => {
      ...
      fail: (err) => {
        uni.showToast({ icon: 'none', title: '服务器请求失败' })
        reject(err)
      },
      complete: (err) => {
        uni.hideLoading()
