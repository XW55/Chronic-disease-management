    ...
    <view style="background-color: #ededed;">
      <view class="item mb20">
        <view class="flexBox">
          ...
          <view class="flexBox">
            <view style="color: #4087eb; margin: 0 15rpx;">{{list.patientName}}【{{list.patientSex}}】{{list.patientAge}}岁
            </view>
      ...
      <view class="item">
        <view class="flexBox">
          ...
          <view class="textHandler">{{list.hospitalName}}</view>
        </view>
      </view>
      ...
      <view class="item mb20">
        <view class="flexBox">
          <uni-icons type="notification" color="#a7a7a7" size="25"></uni-icons>
          <view style="color: #a5a5a5; font-size: 26rpx;">请您于约定时间提前120分钟到分诊台签到候诊，如无法按时就诊请提前取消预约。</view>
          <!-- 此段内容待斟酌 -->
        </view>
      </view>
    </view>
    <view class="btn primary" @click="pay">立即支付（￥{{getPrice(list.chargePrice)}}）</view>
    <view class="btn primary" @click="confirmContent">预约保存</view>
  ...
  export default {
    ...
    methods: {
      async getList() {
        const {
          data: res
        } = await uni.$http.get('/hospital/visitPlan/getPlanMsg', this.params)
      ...
      pay() {
        ...
        uni.showModal({
          title: '支付确认',
          content: '请确保您已到达医院，叫号未到将影响您的就诊，确认立即支付吗',
          success: async function(res) {
            if (res.confirm) {
              const {
                data: ress
