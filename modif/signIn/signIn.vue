  ...
  <view class="container m20 w95 card">
    <u--form :model="userInfo" ref="uForm" :rules="rules">
      ...
      <u-form-item label="证件号码" borderBottom>
        <u-input placeholder="请输入身份证号码" type="number" maxlength="18" v-model="userInfo.patientCode" border="none" @blur="idCardBlur" />
      </u-form-item>
      <u-form-item :required="true" @click="showSex = true" label="性别" prop="sex" borderBottom>
        <u--input v-model="userInfo.sex" disabled disabledColor="#ffffff" placeholder="请选择性别" border="none"></u--input>
      ...
      <picker mode="date" :value="userInfo.birth" @change="birthChange">
        <u-form-item :required="true" label="出生日期" prop="birth" borderBottom>
          <u--input v-model="userInfo.birth" disabled disabledColor="#ffffff" placeholder="请选择生日" border="none"></u--input>
          <u-icon slot="right" name="arrow-right"></u-icon>
        </u-form-item>
      </picker>
      <picker :value="multiIndex" :range="heightColumns" @change="heightChange">
        <u-form-item :required="true" label="身高" prop="height" borderBottom>
          <u--input v-model="userInfo.height" disabled disabledColor="#ffffff" placeholder="请选择身高" border="none"></u--input>
          <u-icon slot="right" name="arrow-right"></u-icon>
        </u-form-item>
      </picker>
      ...
      <picker :value="multiIndex1" :range="weightColumns" @change="weightChange">
        <u-form-item :required="true" label="体重" prop="weight" borderBottom>
          <u--input v-model="userInfo.weight" disabled disabledColor="#ffffff" placeholder="请选择体重" border="none"></u--input>
          <u-icon slot="right" name="arrow-right"></u-icon>
        </u-form-item>
      </picker>
      <!-- 地址选择 -->
    ...
    <!-- 性别选择 -->
    <u-action-sheet :show="showSex" :actions="actions" title="请选择性别" @close="showSex = false" @select="sexSelect" :safeAreaInsetBottom="true"></u-action-sheet>
    <!-- 生日选择 -->
    <u-datetime-picker ref="dateTime" :show="showBirth" itemHeight="100" @cancel="showBirth = false" @confirm="confirmBirth" mode="date" :minDate="-2208988800000"></u-datetime-picker>
    <!-- 提交重置 -->
    <view class="btn_box">
...
<script>
import { formatTimeStamp } from '@/tools/tool.js'
import { initUserInfo, getUserInfoByCode } from '@/request/request.js'
export default {
  data() {
  ...
  methods: {
    ...
    idCardBlur() {
      // 检测身份证是否合格
      if (uni.$u.test.idCard(this.userInfo.patientCode) && this.userInfo.patientCode.length == 18) {
        this.userInfo.birth = this.extractBirthDate(this.userInfo.patientCode)
      }
    },
    // 打开日期
    // openBirth() {
    ...
    async getUserInfo() {
      ...
      if (res && Object.keys(res).length) {
        this.userInfo.name = res.patientName
        this.userInfo.address = res.patientAddress
        this.userInfo.birth = res.birthDay
        this.userInfo.height = res.patientHeight + 'cm'
        this.multiIndex = Number(res.patientHeight - 1)
        this.userInfo.weight = res.patientWeight + 'kg'
        this.multiIndex1 = Number(res.patientWeight - 1)
        this.userInfo.patientCode = res.patientCode
        this.userInfo.sex = res.patientSex == 1 ? '女' : '男'
        this.userInfo.allergy = res.allergies
        this.userInfo.medical = res.patientMedicalHistory
      }
    },
    submit() {
      // 表单验证
      this.$refs.uForm
        ...
        .then(async (res) => {
          // 解构赋值
          const { name, sex, patientCode, height, weight, medical, birth, allergy, address } = this.userInfo
          // 组织信息用于提交表单
          const obj = {
            patientName: name,
          ...
          console.log('用户信息注册成功', result)
          const getInfo = await getUserInfoByCode()
          if (getInfo && Object.keys(getInfo).length) {
            this.$store.commit('user/SET_USERINFO', getInfo)
          }
